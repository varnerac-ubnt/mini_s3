...
s3_simple_request(Config, Method, Host, Path, Subresource, Params, POSTData, Headers) ->
    io:format("s3_request: ~p~n",[[Config, Method, Host, Path,Subresource, Params, POSTData, Headers]]),
...
parse_s3_response({ok, {{StatusCode, Reason}, Headers, Body}}) ->
    Headers1 = canonicalize_headers(Headers),
    case StatusCode >= 200 andalso StatusCode =< 299 of
        true ->
            {Headers1, Body};
        false ->
            Msg = integer_to_list(StatusCode) ++ ":" ++ Reason, 
            erlang:error({aws_error, {http_error, Msg, {Headers,Body}}})
    end.
